  ...
  <div>
    ...
    <el-card class="box-card">
      ...
      <el-table :data="OpenListShow" border stripe>
        ...
        <el-table-column prop="course.course_id" label="课程号"></el-table-column>
        <el-table-column prop="course.course_name" label="课程名"></el-table-column>
        <el-table-column prop="course.credit" label="学分"></el-table-column>
        <el-table-column prop="teacher.user.user_id" label="教师号"></el-table-column>
        <el-table-column prop="teacher.user.user_name" label="教师名"></el-table-column>
...
export default {
  data() {
    return {
      ...
      // 修改表单的验证规则
      editFormRules: {
        semester: [{ required: false, message: '请输入学期', trigger: 'blur' }],
        course_time: [
  ...
  methods: {
    ...
    async getOpenList(search) {
      ...
      await res.data.Opens.forEach((element) => {
        var str = element.semester
        var year = str.slice(0, 4)
        var sem = this.semesterMap[Number(str.slice(5)) - 1]
        element.semester = year + '年' + sem + '季学期'
      })
    ...
    async showEditDialog(row) {
      ...
      const { data: res } = await this.$http.get('admin/open/search/', {
        params: { course: row.course.course_id, teacher: row.teacher.user.user_id },
      })
